...
# В конечном результате должна полуичться MongoDb база данных в которой явно и наглядно понятно, из какого изначального
# файла был добыт номер или несколько номеров.
# Так же должна быть коллекция в БД в которой указаны пути к файлам из которых не удалось извлечь номера с указанием
# страницы если изначально это PDF документ.
